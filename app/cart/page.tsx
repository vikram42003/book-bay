  ...
  return (
    <div className="max-w-7xl mx-auto flex justify-center py-12">
      <div className="bg-white p-8 rounded-xl shadow-lg w-full text-gray-800">
        <h1 className="text-2xl font-bold mb-6 text-center">{user ? `${user.username}'s Cart` : "Your Cart"}</h1>
        <div className="space-y-4">
          {cartItemsArray.length > 0 ? (
            cartItemsArray.map((item: OrderStoreItem) => (
              <div key={item.bookId} className="flex justify-between items-center p-4 border-b">
                <div>
                  <p className="font-semibold">Book ID: {item}</p>
